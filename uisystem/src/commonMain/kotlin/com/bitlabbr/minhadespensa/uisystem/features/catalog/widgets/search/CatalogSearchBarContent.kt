...
@Composable
fun CatalogSearchBarContent(
    query: String,
    onQueryChange: (String) -> Unit,
    searchResults: List<CatalogProductUiModel>,
    onResultClick: (CatalogProductUiModel) -> Unit,
    modifier: Modifier = Modifier,
    placeholder: String = "Buscar produto ou marca..."
) {
        ...
        MinhaDespensaSearchBar(
            ...
            supportingContent = { itemName ->
                ...
                val infoText = when {
                    ...
                    item.brand != null -> "${item.brand} • ${item.category} • ${item.formattedWeight}"
                    else -> "${item.category} • ${item.formattedWeight}"
                }

                MinhaDespensaText(
                    text = infoText,
                    fontStyle = MinhaDespensaTheme.typography.bodySmall,
                    color = getAppColors().onSecondaryContainer.copy(alpha = 0.7f)
                )
            },
            leadingContent = {
                Icon(
